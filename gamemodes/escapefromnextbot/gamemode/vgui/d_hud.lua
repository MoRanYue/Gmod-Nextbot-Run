...
hook.Add("HUDPaint", "nextbotHud", function()
    ...
    if (nbStatus == 1) then
        nbStatus = translate.Get("hud_nbstatus_one")
    elseif (nbStatus == 0) then
        nbStatus = translate.Get("hud_nbstatus_zero")
    end
    ...
    if (NBR.gui.show_nb_info) then
        ...
        draw.SimpleText(translate.Format("hud_nbinfo_status", nbStatus), "DL_Small", 3, 70, Color(255,255,255), 0, 0)
